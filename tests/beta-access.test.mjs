...
test('Pro access requests validate fields, route a replyable email, and report delivery failure',async()=>{
 ...
 // Pro access needs no free session first; pages from before the rename still send "Beta access"
 const valid=await call({topic:'Pro access',from:'test@example.com',xAccount:'https://x.com/user'},'valid');assert.equal(valid.code,200);assert.equal(deliveries[0].reply_to,'test@example.com');assert.match(deliveries[0].text,/X: @user/);assert.match(deliveries[0].text,/Pro access request/);assert.equal(deliveries[0].subject,'OutLoud pro access');
 const legacy=await call({topic:'Beta access',from:'test@example.com',xAccount:'@user'},'legacy');assert.equal(legacy.code,200);assert.match(deliveries[1].text,/Pro password/);
