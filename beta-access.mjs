  ...
  form.addEventListener('submit', async e => {
    ...
    try {
      const response = await fetch('/api/feedback', {method:'POST', headers:{'Content-Type':'application/json'}, signal:controller.signal,
        body:JSON.stringify({topic:'Beta access', from, xAccount:handle, website:form.elements.website.value})});
      ...
      status.textContent = 'Request received. Sean will review it and reply by email with the tester password.';
    } catch (error) {
      button.disabled = false; button.textContent = 'REQUEST BETA ACCESS →';
      ...
      fallback.href = 'mailto:sean@nana.works?subject=OutLoud%20beta%20access&body=' + encodeURIComponent(`I'd like to request OutLoud beta access.\n\nEmail: ${from}\nX: ${handle}`);
