...
function secret(){const key=process.env.SUPABASE_SERVICE_KEY;if(!key)throw fail('Free sessions are temporarily unavailable. Please explore or use your tester password.',503);return key;}
...
async function storage(path,options={}){
 const base=process.env.SUPABASE_URL;if(!base||!/^https:\/\/[^/]+/.test(base))throw fail('Free sessions are temporarily unavailable. Please try again later.',503);
...
export async function reserve(kind,limit=25){
 const day=dayNow();for(let i=0;i<limit;i++)if(await put(`${day}/quota-${kind}-${i}.json`,{at:Date.now()}))return;
 throw fail('Today’s free sessions are fully booked. Please try tomorrow; existing testers can still connect.',429);
}
export async function begin(req,res){
 ...
 if(!await put(`${day}/ip-${ipKey(req)}.json`,{at:Date.now()}))throw fail('A free session has already been started on this network today. Return in the original browser, try tomorrow, or use a tester password.',429);
 try {
  await reserve('sessions',50);
  session={day,id,prefix:day+'/'+id};await put(session.prefix+'-session.json',{created:Date.now()});
 } catch(e) { await del([`${day}/ip-${ipKey(req)}.json`]).catch(()=>{});throw e; }
...
export async function requireSession(req){const s=readSession(req);const record=s?await get(s.prefix+'-session.json'):null;if(!record||!Number.isFinite(record.created)||Date.now()-record.created>=86400000)throw fail('Start your free session first.',401);return s;}
export async function completed(req){const s=await requireSession(req);return !!await get(s.prefix+'-complete.json');}
